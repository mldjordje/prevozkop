...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    ...
    alternates: { canonical: `${process.env.NEXT_PUBLIC_SITE_URL || "https://prevozkop.rs"}/projekti/${params.slug}` },
  };
}
...
export default async function ProjectPage({ params }: Props) {
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-12 space-y-8">
      <Link href="/projekti" className="text-sm text-gray-600 hover:text-primary">← Nazad na projekte</Link>
      <div className="space-y-4">
        <p className="text-primary uppercase tracking-widest text-xs font-semibold">Projekat</p>
        ...
        <div className="space-y-3">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {project.gallery.map((img, idx) => (
              <div key={`${img.src}-${idx}`} className="relative aspect-[4/3] overflow-hidden rounded-lg border border-gray-200 bg-gray-100">
